...
from libs.misc.initial_configs.policy_config import (
    create_policy_from_params,
    create_controller_from_policy,
    create_behavior_policy_from_params,
)

import logger
from libs.misc.data_handling.utils import (
    add_path_data_to_collection_and_update_normalization,
...
from libs.misc.saving_and_loading import (
    save_cur_iter_dynamics_model,
    save_cur_iter_behavior_policy,
    save_cur_iter_policy,
    save_cur_iter_offline_data,
    restore_policy,
    restore_model,
    restore_behavior_policy,
    restore_behavior_policy,
    restore_offline_data,
    confirm_restoring_policy,
    confirm_restoring_dynamics_model,
    confirm_restoring_behavior_policy,
    confirm_restoring_offline_data,
    confirm_restoring_value,
)
...
def get_data_from_random_rollouts(
    params,
    env,
    random_paths,
...
def get_data_from_offline_batch(
    params, env, normalization_scope=None, model="dynamics", split_ratio=0.666667
):
    train_collection = DataCollection(
        batch_size=params[model]["batch_size"],
        max_size=params["max_train_data"],
...
def train_policy_trpo(params, algo, dyn_model, iterations, sess):
    ...
    for j in range(iterations):
        ...
        print(
            "policy optimization time: {}".format(end_policy_update - end_value_eval)
            + "[sec]"
        )
        algo.fit_baseline(paths)
        end_value_fit = time.time()
        print(
            "value fitting time: {}".format(end_value_fit - end_policy_update) + "[sec]"
        )
...
def train(params):
    ...
    if not params["use_offline_dataset"]:
        ...
        ) = get_data_from_random_rollouts(
            params,
            env,
            random_paths,
            normalization_scope="behavior_policy",
            ...
            Path(
                {
                    ...
                    "actions": offline_dataset["actions"][indices[i] : indices[i + 1]],
                    "rewards": offline_dataset["rewards"][indices[i] : indices[i + 1]],
        ...
        (
            ...
            normalization,
            path_collection,
        ) = get_data_from_random_rollouts(params, env, paths, split_ratio=0.85)
    ...
    if params["param_value"]:
        value_variables = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="baseline"
        )
        all_var_except_policy = [
            v
            for v in all_variables
            if v not in (policy_variables + behavior_policy_variables + value_variables)
        ]
    else:
        all_var_except_policy = [
            v
    ...
    logger.log(
        "Train dynamics model with intrinsic reward only? {}".format(
    ...
    if confirm_restoring_value(params):
        algo.baseline.restore_value_function(
            params["restore_path"], params["restart_iter"]
        )
        ...
        if itr % interval == 0:
            if itr != 0:
                ...
                rl_paths = rollout_sampler.sample(
                    num_paths=num_paths, horizon=params["env_horizon"], evaluation=False
                )
            ...
            save_cur_iter_behavior_policy(params, behavior_policy_saver, sess, itr)
            ...
            if params["bc_init"]:
                ...
                logger.info(
                    "TRPO policy initialized with BC average return: {}".format(returns)
                )
            ...
            if params["pretrain_value"]:
                ...
                for obses, _, _, rewards in behavior_policy_train_collection:
                    algo.pre_train_baseline(
                        obses, rewards, params["trpo"]["gamma"], params["trpo"]["gae"]
                    )
        ...
        if params["eval_model"]:
            ...
            traj_wise_mse = np.mean(
                [
                    sum(
                        (
                            np.array(path["observations"])[:traj_len]
                            - np.array(rollout["observations"])[:traj_len]
                        )
                        ** 2
                    )
            ...
            A = np.mean(
                [
                    sum(
                        np.sqrt(
                            (
                                np.array(v[0]["observations"][:traj_len])
                                - np.array(v[1]["observations"][:traj_len])
                            )
                            ** 2
                        )
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random_seeds", type=int, nargs="+", default=[1234, 4321, 2341, 3341, 789]
    )
    parser.add_argument("--n_train", type=int, default=1000000)
    parser.add_argument("--alpha", type=float, default=0.0)  # hyperparam for scaling KL
    parser.add_argument("--target_kl", type=float, default=0.01)  # stepsize of TRPO
    parser.add_argument(
        "--ensemble_model_count", type=int, default=5
    )  # number of dynamics model ensemble
    parser.add_argument(
        "--param_value", action="store_true"
    )  # if true, use parametric value function
    ...
    parser.add_argument("--use_offline_dataset", action="store_true")
